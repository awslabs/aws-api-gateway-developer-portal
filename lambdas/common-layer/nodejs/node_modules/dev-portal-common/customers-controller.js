 ...
 * over-engineered.
 */
async function ensureCustomerItem (cognitoIdentityId, cognitoUserId) {
  let customerItem
  let preLoginItem
  ...
  if (preLoginItem === undefined) {
    throw new Error('Account not found in CustomersTable or PreLoginAccountsTable!')
  }

  const newCustomerItem = await transferPreLoginAccountToCustomersTable({
...
function getCognitoIdentityId (marketplaceCustomerId) {
  const params = {
    TableName: getCustomersTableName(),
  ...
  if (data.Items === undefined || data.Items.length === 0) {
    ...
    throw `No customer is registered in the developer portal for marketplace customer ID ${marketplaceCustomerId}`
  } else {
    return data.Items[0].Id
  }
}

async function subscribe (cognitoIdentityId, usagePlanId) {
  const data = await getApiKeyForCustomer(cognitoIdentityId)
  ...
  if (data.items.length === 0) {
    ...
    const createData = await createApiKey(cognitoIdentityId)
  ...
  return createUsagePlanKey(keyId, usagePlanId)
}

async function unsubscribe (cognitoIdentityId, usagePlanId) {
  const data = await getApiKeyForCustomer(cognitoIdentityId)
  console.log(`Get Api Key data ${JSON.stringify(data)}`)
...
async function __createApiKey ({ identityId, userId, enabled }) {
  ...
  await updateCustomerApiKeyId(identityId, apiKey.id)
  return apiKey
}

async function createApiKey (cognitoIdentityId, cognitoUserId) {
  const isRegistered = await isCognitoUserInGroup({
    userId: cognitoUserId,
    groupName: getEnv('RegisteredGroupName')
  })
  return __createApiKey({
    identityId: cognitoIdentityId,
    userId: cognitoUserId,
...
function createUsagePlanKey (keyId, usagePlanId) {
  console.log(
...
function deleteUsagePlanKey (keyId, usagePlanId) {
  console.log(
  ...
  return apigateway.deleteUsagePlanKey(params).promise()
}
/* eslint-enable standard/no-callback-literal */

function getApiKeyForCustomer (cognitoIdentityId) {
...
async function getUsagePlansForCustomer (cognitoIdentityId) {
  console.log(`Getting API Key for customer ${cognitoIdentityId}`)
  ...
  return { items: await getAllUsagePlans(apigateway, params) }
}

async function getUsagePlanForProductCode (productCode) {
  console.log(`Getting Usage Plan for product ${productCode}`)

  // do a linear scan of usage plans for name matching productCode
  const usagePlans = await getAllUsagePlans(apigateway, { limit: 1000 })
  ...
  const usageplan = usagePlans.find((item) => item.productCode === productCode)
  if (usageplan !== undefined) {
    ...
    throw `Couldn't find usageplan matching product code ${productCode}`
  }
}

async function updateCustomerMarketplaceId (cognitoIdentityId, marketplaceCustomerId) {
  ...
  return updateApiKey(keyId, marketplaceCustomerId)
}

function updateApiKey (apiKeyId, marketplaceCustomerId) {
  console.log(
  ...
  return apigateway.updateApiKey(params).promise()
}
 ...
 */
function updateCustomerApiKeyId (cognitoIdentityId, apiKeyId) {
  ...
  await dynamoDb.update(dynamoDbParams)
